...
BENCHMARK_CONFIGS = {
    "fp32": {
        "precision": "fp32",
        "optimizations": [],
        "description": "Pure FP32 (no optimizations)",
    },
    "fp16": {
        "precision": "fp16",
        "optimizations": [],
...
class KaggleBenchmarkOrchestrator:
    ...
    def __init__(
        ...
        # Generate timestamp for this run's output files
        from datetime import datetime
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        logger.info(f"Benchmark initialized: config={self.config}, timestamp={self.timestamp}")
        logger.info(f"Base results location: {self.base_results_dir.absolute()}")
    ...
    def deploy_model_benchmark(
        ...
        # Create model/config specific output directory
        deployment_results_dir = Path("data/results/kaggle_benchmark") / model_key / self.config
        ...
        try:
            ...
            logger.info(f"  Results location: {deployment_results_dir.absolute()}")
            
            # Note: run_pipeline will handle deploy -> poll -> download internally
            run_pipeline(
                model_id=model_id,
                config=self.config,
                notebook=self.notebook,  # Use custom notebook if provided
    ...
    def load_and_aggregate_results(self) -> bool:
        ...
        for deployment in self.deployments:
            ...
            csv_file = results_dir / "results" / "benchmark_results.csv"
    ...
    def generate_reports(self):
        ...
        # Export timestamped metrics to config-specific directory (CSV only)
        logger.info(f"\n{'='*70}")
        logger.info(f"SAVING METRICS FOR CONFIG: {self.config}")
        logger.info(f"{'='*70}\n")
        
        # Ensure per-config output directories exist
        for model_key in self.models.keys():
            output_dir = self.base_results_dir / model_key / self.config
            output_dir.mkdir(parents=True, exist_ok=True)
            
            try:
                csv_path = output_dir / f"metrics_{self.timestamp}.csv"
                self.collector.export_to_csv(csv_path)
                logger.info(f"Saved: {csv_path.absolute()}")
            except Exception as e:
...
Examples:
  ...
  poetry run python scripts/benchmark.py --mode kaggle --kernel-id-prefix "myusername/my-benchmark"
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to custom notebook file (relative to project root)"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=3,
        help="Limit number of images to run per iteration (default: 3, use 0 or 20 for all)"
    )
